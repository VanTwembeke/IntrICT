...
export async function PUT(request: Request, { params }: Params) {
  ...
  const { status, notes, guest_name, guest_email, guest_phone, guest_company } = body;

  const db = createAdminClient();

  const { data: current } = await db.from('client_dossiers').select('status, profile_id').eq('id', id).single();
  ...
  if (guest_company !== undefined) updates.guest_company = guest_company;
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const db = createAdminClient();
  const { error } = await db.from('client_dossiers').delete().eq('id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  if (action === 'add_package') {
    const { data: dossier } = await db.from('client_dossiers').select('profile_id').eq('id', id).single();
    const { data, error } = await db
  ...
  if (action === 'remove_package') {
    const { error } = await db.from('dossier_packages').delete().eq('id', package_id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ ok: true });
  }
