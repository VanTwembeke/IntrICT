...
import {
  ...
  Users, Mail, MousePointerClick, TrendingUp, Check, AlertCircle,
} from 'lucide-react';
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit mb-6">
        {([['compose', 'Nieuwe nieuwsbrief', <Send key="s" size={14} />], ['history', `Geschiedenis (${totalSent})`, <History key="h" size={14} />]] as const).map(([id, label, icon]) => (
          <button
            key={id}
            onClick={() => setTab(id)}
